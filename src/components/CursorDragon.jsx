...
function DragonScene({ cursorTarget, enabled }) {
  const [segments, setSegments] = useState(() =>
    Array.from({ length: N }, () => ({ x: 0, y: 0, angle: 0, scale: 1 }))
  )
  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      const nextSegments = segmentsRef.current.map((entry) => ({ ...entry }))
      const pointer = {
        x: cursorTarget.x * viewBoxWidth + viewBoxXMin,
        y: cursorTarget.y * viewBoxHeight + viewBoxYMin,
      }
      ...
      first.x += (ax + pointer.x - first.x) / 10
      first.y += (ay + pointer.y - first.y) / 10

      for (let i = 1; i < N; i += 1) {
        const current = nextSegments[i]
        const previous = nextSegments[i - 1]
        const angle = Math.atan2(current.y - previous.y, current.x - previous.x)
        current.x += (previous.x - current.x + (Math.cos(angle) * (100 - i)) / 22) / 4
        current.y += (previous.y - current.y + (Math.sin(angle) * (100 - i)) / 22) / 4
        current.angle = angle
        current.scale = (162 + 4 * (1 - i)) / 300
      }

      segmentsRef.current = nextSegments
      setSegments(nextSegments)

      if (rad < Math.min(viewBoxWidth, viewBoxHeight) / 6 - 20) {
        rad += 1
      }
      frm += 0.003
  ...
  }, [cursorTarget.x, cursorTarget.y, enabled])
  ...
  return (
    <>
      <svg
        ...
        className="dragon-svg-fallback"
        aria-hidden="true"
      >
        <circle cx="0" cy="0" r="48" fill="rgba(255,255,255,0.14)" />
        <path
          d="M-24 -12 C-8 -36, 18 -38, 36 -18 C48 -8, 44 12, 24 20 C8 28, -8 24, -16 12 C-18 8, -24 4, -28 -2"
          fill="none"
          stroke="#f8fafc"
          strokeWidth="10"
          strokeLinecap="round"
        />
        <path d="M-14 -12 Q-30 -26 -44 -12" fill="none" stroke="#8b5cf6" strokeWidth="8" strokeLinecap="round" />
        <path d="M20 -8 Q38 -20 48 -4" fill="none" stroke="#38bdf8" strokeWidth="8" strokeLinecap="round" />
        <circle cx="-12" cy="-2" r="4" fill="#0f172a" />
      </svg>
      <svg
        viewBox="-150 -150 300 300"
        preserveAspectRatio="xMidYMid meet"
        ...
        role="img"
        aria-label="Animated dragon following the cursor"
      >
      <defs>
        ...
        <g id="Aletas" transform="matrix(1, 0, 0, 1, 0, 0)">
          <linearGradient id="LinearGradID_1" gradientUnits="userSpaceOnUse" x1="-819.2" y1="0" x2="819.2" y2="0">
        ...
        <g id="Espina" transform="matrix(1, 0, 0, 1, 0, 0)">
          <linearGradient id="LinearGradID_2" gradientUnits="userSpaceOnUse" gradientTransform="matrix(0.0935974, 0, 0, 0.188782, -20.55, 0)" spreadMethod="pad" x1="-819.2" x2="819.2" y1="0" y2="0">
            <stop offset="0" style={{ stopColor: "#cccccc", stopOpacity: 1 }} />
          ...
          />
          <linearGradient id="LinearGradID_3" gradientUnits="userSpaceOnUse" gradientTransform="matrix(0.0229492, 0, 0, 0.0152893, 0, -0.05)" spreadMethod="pad" x1="-819.2" x2="819.2" y1="0" y2="0">
            <stop offset="0" style={{ stopColor: "#cccccc", stopOpacity: 1 }} />
            <stop offset="1" style={{ stopColor: "#333333", stopOpacity: 1 }} />
          </linearGradient>
          <path
      ...
      {segments.slice(1).map((segment, index) => {
        ...
        const previous = segments[renderIndex - 1]
        const x = (previous.x + segment.x) / 2
        const y = (previous.y + segment.y) / 2
        const transform = `translate(${x},${y}) rotate(${(180 / Math.PI) * segment.angle}) scale(${segment.scale},${segment.scale})`
        const useId = renderIndex === 1 ? "Cabeza" : renderIndex === 6 || renderIndex === 13 ? "Aletas" : "Espina"
        return <use key={`${renderIndex}-${segment.x}-${segment.y}`} href={`#${useId}`} transform={transform} />
      })}
    </svg>
    </>
  )
...
export default function CursorDragon() {
  const { target } = useCursorTarget()

  return (
    <div className="dragon-overlay" aria-hidden="true">
      <div className="dragon-canvas-shell">
        <DragonScene cursorTarget={target} enabled />
      </div>
    </div>
  )
}
